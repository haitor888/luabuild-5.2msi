...
function reconnect()
    outf:write("attempting data connection... ")
...
function accept_timeout()
    outf:write("accept with timeout (if it hangs, it failed): ")
...
function connect_timeout()
    outf:write("connect with timeout (if it hangs, it failed!): ")
    local t = socket.gettime()
...
function connect_errors()
    outf:write("connection refused: ")
    local c, e = socket.connect("localhost", 1);
    assert(not c and e)
    print("ok: ", e)
    outf:write("host not found: ")
